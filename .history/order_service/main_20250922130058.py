...
async def create_order(
    ...
    if not customer:
        raise HTTPException(status_code=400, detail="Invalid Customer", detail="The customer ID provided is invalid.")

    # Validate product
    product = await validate_product(order.product_id, token)
    if not product:
        raise HTTPException(status_code=400, detail="InvalidProduct", detail="The product ID provided is invalid.")

    # Validate pricing
    pricing = await validate_pricing(order.product_id, token)
    if not pricing:
        raise HTTPException(status_code=400, detail="PricingNotFound", detail=f"No pricing found for product_id={order.product_id}")
