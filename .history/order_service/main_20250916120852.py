...
async def create_order(order: schemas.OrderCreate, db: Session = Depends(get_db)):
    ...
    if not product:
        raise HTTPException(status_code=400, detail="Invalid product ID")
    pricing = await get_product_price(order.product_id)
    if not pricing:
        raise HTTPException(status_code=400, detail="Failed to fetch product pricing")
    order.total_amount = pricing.get("price", 0) * order.quantity
    return crud.create_order(db, order)
