...
# ---------- Validators ----------
async def validate_customer(customer_id: int, request: Request):
    headers = get_forward_headers(request)
    async with httpx.AsyncClient() as client:
        response = await client.get(f"{CUSTOMER_SERVICE_URL}/{customer_id}", headers=headers)
    return response.json() if response.status_code == 200 else None


async def validate_product(product_id: int, request: Request):
    headers = get_forward_headers(request)
    async with httpx.AsyncClient() as client:
...
async def validate_pricing(product_id: int, request: Request):
...
async def update_inventory(product_id: int, quantity: int, request: Request):
    headers = {"Authorization": request.headers.get("Authorization")}
    async with httpx.AsyncClient() as client:
        try:
...
async def create_order(order: schemas.OrderCreate, request: Request, db: Session = Depends(get_db)):
    # Validate customer
    customer = await validate_customer(order.customer_id, request)
    if not customer:
        raise HTTPException(status_code=400, detail="InvalidCustomer")

    # Validate product
    product = await validate_product(order.product_id, request)
    if not product:
        raise HTTPException(status_code=400, detail="InvalidProduct")

    # Validate pricing
    pricing = await validate_pricing(order.product_id, request)
    if not pricing:
    ...
    inventory = await update_inventory(order.product_id, order.quantity, request)
    if not inventory:
...
async def list_orders(request: Request, db: Session = Depends(get_db)):
    ...
    for db_order in db_orders:
        pricing = await validate_pricing(db_order.product_id, request)
        if not pricing:
...
async def update_order(order_id: int, order: schemas.OrderUpdate, request: Request, db: Session = Depends(get_db)):
    ...
    restore_inventory = await update_inventory(existing_order.product_id, -existing_order.quantity, request)
