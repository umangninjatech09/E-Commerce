...
def api_create_pricing(pricing: schemas.PricingCreate, db: Session = Depends(get_db)):
    product = db.query(models.Product).filter(models.Product.id == pricing.product_id).first()
    if not product:
        return error_response(404, "ProductNotFound",
            f"Cannot create pricing: product_id={pricing.product_id} does not exist."
        )
