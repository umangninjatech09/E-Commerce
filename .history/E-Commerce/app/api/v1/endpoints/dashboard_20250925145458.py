...
ORDER_SERVICE_user = "http://order_service:8001/api/frequent-users"
ORDER_SERVICE_product = "http://order_service:8001/api/frequent-products"
...
async def products_summary(
    ...
    # 1. Total number of products (from ecommerce DB)
    total_products = db.query(func.count(Product.id)).scalar() or 0

    # 2. Fetch the frequent products from the Order Microservice
    async with httpx.AsyncClient() as client:
        response = await client.get(
            ORDER_SERVICE_product,
            params={"threshold": threshold}  # Pass the threshold to the order service
        )
        ...
        frequent_products = response.json().get("frequent_products", 0)
